...
class StyleLoss(nn.Module):
    ...
    def forward(self, input):
        if self.mode == "loss":
            self.loss = F.mse_loss(gram_matrix(input, self.feature_norm), self.target)
        elif self.mode == "target":
...
class VGG19Loss(nn.Module):
    def __init__(
        ...
        self.style_weights = {
            layer: weight / sum(style_weights.values())
            for layer, weight in style_weights.items()
        }
        self.content_weights = {
            layer: weight / sum(content_weights.values())
    ...
    def forward(self, input):
        ...
        return (total_loss, content_loss, style_loss, content_losses, style_losses)
    ...
    def reset(self):
        for loss in chain(self.content_losses.values(), self.style_losses.values()):
            if hasattr(loss, "target"):
                delattr(loss, "target")
    ...
    def _build_vgg_loss(self, avg_pool, feature_norm, weights, device):
        ...
        if weights in ("original", "normalized"):
            state_dict = load_state_dict_from_url(
                "https://storage.googleapis" f".com/prism-weights/vgg19-{weights}.pth"
            )
        else:
        ...
        for layer in vgg.children():
            ...
            if name in self.content_weights:
                ...
                self.vgg_loss.add_module(
                    f"content_loss_{i_pool}_{i_conv}", content_loss
                )
                self.content_losses[name] = content_loss
            if name in self.style_weights:
                style_loss = StyleLoss("none", feature_norm)
                self.vgg_loss.add_module(f"style_loss_{i_pool}_{i_conv}", style_loss)
                self.style_losses[name] = style_loss
            if len(self.style_weights) == len(self.style_losses) and len(
                self.content_weights
            ) == len(self.content_losses):
                break
